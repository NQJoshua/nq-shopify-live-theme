...
Shopify.onCartUpdate = function(cart) {
  ...
  Shopify.cartUpdateInfo(cart, '.cart-group-1 ul.cart-items');
};
...
Shopify.cartUpdateInfo = function(cart, cart_cell_id) {
  ...
  var formatMoneyNoCurr = "<span>{{amount}}</span>";
        ...
        if (key === 'items') {
          ...
          if (value.length) {
            ...
            jQuery.each(value, function(i, item) {
              ...
              jQuery('<li class="cart__item row"><div class="col col-md-1"><div class="cart__item__image"> <a href="' + item.url + '"><img src="' + item.image + '" alt=""/></a> </div></div><div class="col col-md-4"><div class="cart__item__info__title"><h2><a href="' + item.url + '">' + item.title + '</a></h2></div></div><div class="col col-xs-3 col-md-2"><div class="hidden-md hidden-lg text-muted">Price</div><div class="cart__item__info__price"><span>' + Shopify.formatMoney(item.price, formatMoney) + '</span></div></div><div class="col col-xs-3 col-md-2"><div class="hidden-md hidden-lg text-muted">Qty</div><div class="cart__item__info__price" style="right: 35%;"><input type="number" value="' + item.quantity + '" style="width: 70px;" onchange="setQuantity(' + item.variant_id + ',this.value,' + item.price + ')" /></div></div><div class="col col-xs-3 col-md-2"><div class="hidden-md hidden-lg text-muted">Total</div><div class="cart__item__info__price"><span id="itemTotal-' + item.variant_id + '">' + Shopify.formatMoney(item.price * item.quantity, formatMoney) + '</span></div></div><div class="col col-md-1"><div class="cart__item__delete"><a href="javascript:void(0);" onclick="Shopify.removeItem(' + item.variant_id + ')" class="icon icon-delete"><span>' + jQuery(".cart_messages .delete").text() + '</span></a></div></div></li>').appendTo(table);
            });
          }
          else {
  ...
  changeHtmlValue(".shopping-cart__total", Shopify.formatMoney(cart.total_price, formatMoneyNoCurr));
